...
type Sheet = null | "create" | "quote" | "invoice" | "receipt" | "accounting" | "analyze" | "credits" | "customer" | "job" | "team";
    ...
    <main className="app-shell">
      ...
      {sheet && <div className="overlay" onMouseDown={e => e.target === e.currentTarget && setSheet(null)}>
        <section className={`sheet ${sheet === "quote" ? "wide" : ""}`} role="dialog" aria-modal="true">
          ...
          {sheet === "team" && <TeamSheet notify={notify} />}
        </section>
      </div>}
...
function HomeView({ setSheet, credits, paymentReminder }: { setSheet: (s: Sheet) => void; credits: number; paymentReminder: { due: string; terms: string } | null }) {
  return <>
    ...
    <section className="quick-grid">
      ...
      <button className="quick-card" onClick={() => setSheet("invoice")}><span className="quick-icon green">I</span><span><strong>Make & send invoice</strong><small>Request customer payment</small></span><i>→</i></button>
      <button className="quick-card" onClick={() => setSheet("receipt")}><span className="quick-icon amber">R</span><span><strong>Record payment</strong><small>Create customer receipt</small></span><i>→</i></button>
    </section>
...
function MoreView({ credits, setSheet }: { credits: number; setSheet: (s: Sheet) => void }) { return <section className="settings-grid"><section className="panel account-card"><span className="avatar large">AT</span><div><h2>Ahmad Teknik Services</h2><p>Ahmad bin Ismail · Johor Bahru</p><span className="plan-tag">OWNER / MASTER ACCOUNT</span></div><button className="secondary">Edit profile</button></section><section className="panel accounting-summary"><div><span className="quick-icon green">RM</span><div><h3>Money & accounting</h3><p>Keep sales, payments, receipts and business details organised.</p></div></div><div className="accounting-numbers"><span><small>INVOICED</small><b>RM2,940</b></span><span><small>COLLECTED</small><b>RM2,510</b></span><span><small>OUTSTANDING</small><b>RM430</b></span></div><button className="primary" onClick={() => setSheet("accounting")}>Open accounting setup</button></section><section className="panel settings-list"><h3>Business</h3>{["Business profile & logo", "Document defaults", "Payment details", "Language: English"].map(x => <button key={x}><span>▧</span>{x}<b>›</b></button>)}<button onClick={() => setSheet("accounting")}><span>RM</span>Accounting & tax details <em>Setup needed</em><b>›</b></button><button onClick={() => setSheet("team")}><span>♙</span>Team & staff accounts <em>Solo / Team</em><b>›</b></button><h3>Account</h3><button onClick={() => setSheet("credits")}><span>✦</span>Plan & AI Credits <em>{credits} left</em><b>›</b></button><button><span>♢</span>Reminders<b>›</b></button><button><span>?</span>Help & support<b>›</b></button></section><p className="legal-note">Privacy Policy · Terms of Service<br/><small>Drafts for legal review · KerjaPro v0.1</small></p></section>; }

function CreateSheet({ choose }: { choose: (s: Sheet) => void }) { return <><div className="sheet-head"><span className="eyebrow">CREATE NEW</span><h2>What do you want to make?</h2><p>You can type it yourself for free, or let AI help.</p></div><div className="create-options"><button onClick={() => choose("quote")}><span className="quick-icon blue">▤</span><div><strong>Quotation</strong><small>Price and scope for a customer</small></div><b>›</b></button><button><span className="quick-icon green">✓</span><div><strong>Completion Report</strong><small>Record work completed</small></div><b>›</b></button><button onClick={() => choose("invoice")}><span className="quick-icon amber">I</span><div><strong>Invoice</strong><small>Request payment and share with customer</small></div><b>›</b></button><button onClick={() => choose("receipt")}><span className="quick-icon green">R</span><div><strong>Payment Receipt</strong><small>Confirm customer payment received</small></div><b>›</b></button><button onClick={() => choose("analyze")}><span className="quick-icon purple">▰</span><div><strong>Analyze Customer Message</strong><small>Understand WhatsApp text or voice</small></div><b>›</b></button></div><p className="sheet-foot">Manual documents are always free. AI assistance uses 1 AI Credit.</p></>; }
...
function InvoiceSheet({ notify }: { notify: (s: string) => void }) {
  ...
  const invoiceLines = ["INVOICE", "Ahmad Teknik Services", "Johor Bahru, Malaysia", "Phone: 012-345 6789", "", "Invoice No: INV-2026-0029", `Date: ${new Date().toLocaleDateString("en-MY")}`, "Due: On receipt", "", "BILL TO", "Mr Ravi", "Permas Jaya, Masai", "", "DESCRIPTION                                      QTY       RATE       AMOUNT", "Aircon chemical wash                              1      RM180.00     RM180.00", "--------------------------------------------------------------------------", "TOTAL                                                              RM180.00", "", "Payment details: Bank transfer / cash", "Thank you for your business."];
  ...
  const sharePhone = async () => {
    ...
    download(); window.open(`https://wa.me/${phone.replace(/\D/g, "")}?text=${encodeURIComponent("Hi, here is invoice INV-2026-0029 for RM180.00 from Ahmad Teknik Services. The PDF has been downloaded—please attach it in this chat.")}`, "_blank", "noopener,noreferrer");
  };
  if (confirmed) return <><div className="success-screen invoice-success"><span>✓</span><h2>Invoice ready to send</h2><p>INV-2026-0029 · Mr Ravi · RM180.00</p></div><div className="send-options"><button className="whatsapp" onClick={() => void sharePhone()}><b>▰ Share by WhatsApp</b><small>Uses phone share sheet when supported</small></button><button onClick={() => { download(); window.location.href = `mailto:${email}?subject=${encodeURIComponent("Invoice INV-2026-0029")}&body=${encodeURIComponent("Hi, please find invoice INV-2026-0029 for RM180.00. Attach the downloaded PDF before sending.")}`; }}><b>✉ Send by email</b><small>Downloads PDF, then opens your email app</small></button><button onClick={() => { download(); notify("Invoice PDF downloaded"); }}><b>↓ Download PDF</b><small>Share it yourself</small></button></div><div className="privacy-callout">KerjaPro opens your chosen app. You review and press Send yourself.</div></>;
  return <><div className="sheet-head"><span className="eyebrow">NEW INVOICE</span><h2>Check invoice details</h2><p>Create it free, confirm it, then choose how to send it.</p></div><div className="form-grid"><label>Customer<input defaultValue="Mr Ravi" /></label><label>Invoice number<input defaultValue="INV-2026-0029" /></label><label>WhatsApp number<input value={phone} onChange={e => setPhone(e.target.value)} /></label><label>Email<input value={email} onChange={e => setEmail(e.target.value)} /></label><label className="full">Description<input defaultValue="Aircon chemical wash" /></label><label>Amount (RM)<input defaultValue="180.00" /></label><label>Payment due<select defaultValue="receipt"><option value="receipt">On receipt</option><option value="3">Within 3 days</option><option value="30">Within 30 days</option></select></label></div><div className="invoice-total"><span>Total</span><strong>RM180.00</strong></div><label className="check"><input type="checkbox" defaultChecked /> I checked the customer, amount and payment details.</label><button className="primary full-button" onClick={() => setConfirmed(true)}>Confirm invoice</button><p className="human-note">The invoice is never sent automatically.</p></>;
}
...
function AccountingSheet({ notify }: { notify: (s: string) => void }) {
  ...
  return <><div className="sheet-head"><span className="eyebrow">MONEY & ACCOUNTING SETUP</span><h2>Keep the right business details</h2><p>These fields improve your records and documents. Ask your accountant or tax agent when unsure.</p></div><div className="accounting-tabs"><button className="active">Business details</button><button>Money records</button><button>e-Invoice readiness</button></div><div className="form-grid"><label>Legal / business name<input defaultValue="Ahmad Teknik Services" /></label><label>SSM registration no.<input placeholder="Optional for unregistered business" /></label><label>Tax Identification No. (TIN)<input placeholder="e.g. IG / C number" /></label><label>MSIC code<input placeholder="Business activity code" /></label><label className="full">Business activity description<input defaultValue="Air-conditioning installation, service and repair" /></label><label>SST status<select value={sst} onChange={e => setSst(e.target.value)}><option value="not-registered">Not registered / not sure</option><option value="registered">SST registered</option></select></label>{sst === "registered" && <label>SST registration no.<input placeholder="Enter SST number" /></label>}<label>Financial year end<select defaultValue="dec"><option value="dec">31 December</option><option value="other">Other</option></select></label><label>Default currency<input defaultValue="MYR" /></label></div><div className="record-checklist"><h3>Records to keep together</h3><div><span>✓</span><b>Sales</b><small>Invoices, receipts, credit notes</small></div><div><span>✓</span><b>Expenses</b><small>Supplier bills and original receipts</small></div><div><span>✓</span><b>Payments</b><small>Cash, bank, DuitNow and card references</small></div><div><span>✓</span><b>Bank check</b><small>Match collected amounts to statements</small></div></div><div className="compliance-note"><b>Important: KerjaPro documents are not MyInvois submissions</b><p>A normal PDF invoice or receipt is not automatically an LHDN e-Invoice. MyInvois integration and validation must be implemented separately. SST fields should only be shown when the business is actually registered.</p><div><a href="https://www.hasil.gov.my/en/e-invoice/reference-for-the-implementation-of-e-invoice/guidelines/" target="_blank" rel="noreferrer">LHDN e-Invoice guidance ↗</a><a href="https://mysst.customs.gov.my/issuing-invoices/" target="_blank" rel="noreferrer">MySST invoice guidance ↗</a></div></div><div className="retention-note"><span>7</span><div><b>Record-retention reminder</b><p>Official tax guidance generally requires supporting business records to be kept for seven years. Confirm what applies to your business.</p></div></div><button className="primary full-button" onClick={() => notify("Accounting details saved")}>Save accounting details</button></>;
}

function TeamSheet({ notify }: { notify: (s: string) => void }) {
  const [mode, setMode] = useState<"solo" | "team">("team");
  const [invite, setInvite] = useState(false);
  return <><div className="sheet-head"><span className="eyebrow">ACCOUNT TYPE</span><h2>How do you work?</h2><p>You can change this later. The master account keeps control of billing and business settings.</p></div><div className="account-choice"><button className={mode === "solo" ? "active" : ""} onClick={() => setMode("solo")}><span>♙</span><b>Just me</b><small>One owner account</small></button><button className={mode === "team" ? "active" : ""} onClick={() => setMode("team")}><span>♙♙</span><b>I have a team</b><small>Owner plus staff sub-accounts</small></button></div>{mode === "solo" ? <div className="solo-note"><b>Solo account selected</b><p>You can create a team whenever you hire staff.</p></div> : <><div className="owner-card"><span className="avatar">AT</span><div><b>Ahmad bin Ismail</b><small>MASTER ACCOUNT · Full access</small></div><em>Owner</em></div><div className="team-heading"><div><h3>Staff accounts</h3><p>See assigned jobs and work progress.</p></div><button className="primary" onClick={() => setInvite(true)}>＋ Add staff</button></div>{invite && <div className="invite-form"><label>Staff name<input placeholder="e.g. Hafiz" /></label><label>Mobile or email<input placeholder="012-345 6789" /></label><label>Permission<select><option>Worker — own assigned jobs</option><option>Supervisor — all jobs</option></select></label><button className="primary" onClick={() => { setInvite(false); notify("Staff invitation prepared"); }}>Send invite</button></div>}<div className="staff-list"><div><span className="customer-avatar blue">HF</span><span><b>Hafiz</b><small>Last active 18 minutes ago</small></span><i>2 assigned</i><em>1 completed</em></div><div><span className="customer-avatar gold">KL</span><span><b>Kumar</b><small>Last active yesterday</small></span><i>1 assigned</i><em>3 completed</em></div></div><div className="permissions"><h3>Staff can</h3><p>✓ View and update jobs assigned to them<br/>✓ Add work notes and completion photos<br/>✓ Create draft work reports</p><h3>Owner only</h3><p>🔒 Billing, plans and AI Credit purchases<br/>🔒 Business profile and staff management<br/>🔒 Revenue and all-customer reports</p></div></>}<button className="primary full-button" onClick={() => notify(mode === "team" ? "Team account settings saved" : "Solo account settings saved")}>Save account type</button></>;
}
